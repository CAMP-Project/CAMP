...
MMArbitrator_NS::MMArbitrator_NS(ros::NodeHandle n) 
{
    ...
    this->_odom_sub = n.subscribe("odom", 10, &MMArbitrator_NS::_position_callback, this);
    this->_map_sub = n.subscribe("map", 10, &MMArbitrator_NS::_map_callback, this);
    ...
    if (this->_mm_entry_point.call(this->_fkie_service))
    {
        ...
        for (int i = 0; i < s; i++)
        {
            ...
            if(std::find(this->_available.begin(), this->_available.end(), new_host ) == this->_available.end())
            {
                ...
                // ros::V_Subscriber::iterator ch = std::find_if(this->_subs.begin(), this->_subs.end(), boost::bind(&MMArbitrator_NS::_sub_comp, this, _1, new_host+"/odom" ) );
 ...
 */
void MMArbitrator_NS::_position_callback(const nav_msgs::Odometry::ConstPtr& pos)
{ 
 ...
 * @param map 
 */
void MMArbitrator_NS::_map_callback(const nav_msgs::OccupancyGrid::ConstPtr& map)
{ 
...
void MMArbitrator_NS::update_host_list()
{
    ...
    // Clear the host_list array.
    std::string empty_list[this->_available.size()];
    this->_host_list = new brian::RobotKeyList(this->_available.size());
    ...
    for (int i = 0; i < this->_available.size(); i++) 
    {
        ...
        this->_host_list.robotKeys[i] = element;
