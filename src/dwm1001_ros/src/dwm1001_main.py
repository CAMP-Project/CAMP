...
# Filter coefficients
coefficient = [-0.0053,-0.0028,0.0435,0.1695,0.2951,0.2951,0.1695,0.0435,-0.0028,-0.0053]
...
# Function to filter location data
def filter(data, coefficients): 
    # Turns data into a vector
    x = math.array(data) 
    coefficients = math.array(coefficients)

    # Gives sum of products of the data and coefficients
    x_filt = math.dot(x, coefficients)
...
class dwm1001_localizer:
    ...
    def pubblishCoordinatesIntoTopics(self, networkDataArray):
        ...
        for network in networkDataArray:
	        ...
	        x_queue.append(float(networkDataArray[networkDataArray.index(network) + 1]))
                y_queue.append(float(networkDataArray[networkDataArray.index(network) + 2]))
                z_queue.append(float(networkDataArray[networkDataArray.index(network) + 3]))
                temp1 = len(x_queue)
                ...
                # Uses filtered values 
                    ...
                    tag = Tag(float(filter(x_queue, coefficient)), 
